...
There are approximately 26^2 possible combinations of alternating pairs of letters. Notice that this number remains constant and is not dependent on the length of our input String. This fact will help us achieve a linear O(n) runtime.
...
- int[26][26] letter ---> This array is used to keep track of which of the alternating characters we saw last. To achieve this, for the current character "ch", we update the corresponding row (26 entries) and column (26 entries) with the (ASCII) value of "ch".
- int[26][26] count ----> if we find that no solution exists for a pair of characters (which happens when the characters don't alternate), we store -1 in this array. Otherwise, we store he current maximum length of "s" for the pair of characters.

Our final answer is the maximum value in "int[26][26] count".
